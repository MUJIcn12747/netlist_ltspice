...
from parameters import maxConductance, minConductance, AM_or_QM, error_range_AM, num_Bit, sigma_QW, unit_Current, unit_Voltage, alpha_mvm, alpha_inv

def MVM_RealDevice_Update(A, N, M):
...
def InputMapping_mvm(V_in):
    V_in = V_in * alpha_mvm / np.max(V_in)
    return V_in

def InputMapping_inv(vector_b):
    Voltage = (-1) * vector_b * alpha_inv / np.max(vector_b)
    return Voltage

def Mapping_mvm_pos(N, M, A):
    G_actual = MVM_RealDevice_Update(A,N,M)       # attach device model to conductance
